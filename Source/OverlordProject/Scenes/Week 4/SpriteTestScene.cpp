...
void SpriteTestScene::Initialize()
{
	...
	m_pSprite->AddComponent(new SpriteComponent(L"Textures/TestSprite.jpg", { .5f , .5f }, { 1.f, 1.f, 1.f, .5f }));
	AddChild(m_pSprite);

	m_pSprite->GetTransform()->Translate(m_SceneContext.windowWidth * .5f, m_SceneContext.windowHeight * .5f, .9f);
	m_pSprite->GetTransform()->Scale(1.f, 1.f, 1.f);
}

void SpriteTestScene::Update()
{
	m_pSprite->GetTransform()->Rotate(.0f, .0f, m_TotalRotation);
